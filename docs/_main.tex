...
%
\Abstract{
  This \emph{R Markdown} template is for writing an Harrisburg University dissertation.
  The template is built using Yihui Xie's \texttt{bookdown} package, with heavy inspiration from Chester Ismay's \texttt{thesisdown} and the \texttt{OxThesis} \LaTeX~template (most recently adapted by John McManigle).
...
Then you can reference that variable as the value of \texttt{out.extra} to rotate images, as in Figure \ref{fig:HarrisburgU-logo-rotated}.
...
This chapter describes a number of possible customizations to the \texttt{stormdown} dissertation.
...
Therefore, when you build your thesis to PDF, split the PDF into an alphanumerically sorted sequence of \textbf{single-page} PDF files (you can do this automatically with the package \texttt{pdftools}).
You can then use the appropriate LaTeX command to insert them, as shown below (for brevity, in the \texttt{stormdown} PDF sample content we're only including two pages)
...
\CommentTok{# 1.0 is 100 per cent of the stormdown PDF page width;}
...
The \texttt{stormdown} package uses biblatex in latex for referencing.
...
The following code, when it replaces the existing correpsonding code block in \textbf{HUclass.cls}, puts chapter number and title centered in the header and page number in the footer, centered.
