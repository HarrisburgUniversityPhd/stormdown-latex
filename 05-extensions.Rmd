...
This chapter describes a number of possible customizations to the `stormdown` dissertation. 
...
Therefore, when you build your thesis to PDF, split the PDF into an alphanumerically sorted sequence of **single-page** PDF files (you can do this automatically with the package `pdftools`).
You can then use the appropriate LaTeX command to insert them, as shown below (for brevity, in the `stormdown` PDF sample content we're only including two pages)
...
# 1.0 is 100 per cent of the stormdown PDF page width;
...
The `stormdown` package uses biblatex in latex for referencing. 
...
The following code, when it replaces the existing correpsonding code block in **HUclass.cls**, puts chapter number and title centered in the header and page number in the footer, centered. 
